...
@Plugin(
		...
		visible=true,
		description="Treat an image object as a stack and calculate the mean stack intensity."
		)
public class CTC_MakeCalibrationImageFromObject extends JEXPlugin{
	...
	@InputMarker(name="Source Images", type=MarkerConstants.TYPE_IMAGE, description="Images to be used for calibration", optional=false)
	JEXData imageData;
	...
	@ParameterMarker(uiOrder=1, name="Pseudo Median Subroup Size", description="The number of images in each subgroup. Used for pseudo median option. Each median operation only produces integer value increments. Mean produces decimal increments", ui=MarkerConstants.UI_TEXTFIELD, defaultText="10")
	int groupSize;
	...
	public boolean run(JEXEntry optionalEntry)
	{
		...
		output = ImageWriter.makeImageObject(output.name, "Placeholder");
